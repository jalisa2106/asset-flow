...
export default function DashboardPage() {
  ...
  const getActionIcon = (action: string) => {
    ...
    if (act.includes("allocat") || act.includes("transfer")) return RotateCcw;
    if (act.includes("book")) return CalendarDays;
    if (act.includes("audit") || act.includes("flag") || act.includes("overdue")) return AlertCircle;
    return CheckCircle2;
  };

  const getActionColor = (action: string) => {
    const act = action.toLowerCase();
    if (act.includes("allocat")) return "text-positive";
    if (act.includes("transfer")) return "text-blue-500";
    if (act.includes("audit") || act.includes("flag") || act.includes("overdue")) return "text-destructive";
    if (act.includes("book")) return "text-purple-500";
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-border bg-card shadow-sm p-4">
              ...
              {data.mine?.allocations && data.mine.allocations.length > 0 ? (
                <ul className="space-y-2">
                  {data.mine.allocations.map((a: any) => (
            ...
            <div className="rounded-xl border border-border bg-card shadow-sm p-4">
              ...
              {data.mine?.bookings && data.mine.bookings.length > 0 ? (
                <ul className="space-y-2">
                  {data.mine.bookings.map((b: any) => (
            ...
            <div className="rounded-xl border border-border bg-card shadow-sm p-4">
              ...
              {data.mine?.maintenance && data.mine.maintenance.length > 0 ? (
                <ul className="space-y-2">
                  {data.mine.maintenance.map((m: any) => (
