...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (decision === 'Rejected') {
    ...
    const res = await supabase.rpc('approve_transfer', {
      p_transfer_id: id,
  ...
  await supabase.rpc('log_activity', {
    p_actor_id: profile!.id, p_action: `transfer.${decision === 'Rejected' ? 'rejected' : 'approved'}`, p_entity_type: 'transfer_request', p_entity_id: id,
    p_metadata: { decision },
  });
